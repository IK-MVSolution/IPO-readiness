...
| Key | Value | หมายเหตุ |
|-----|-------|----------|
| `FRONTEND_URL` | `https://your-vercel-app.vercel.app` | Optional |
| **`DATABASE_URL`** | *(ดูขั้นตอนด้านล่าง)* | **สำคัญ** – ถ้าไม่ตั้ง ข้อมูล User จะหายทุกครั้งที่เซิร์ฟเวอร์ restart |

### 🗄️ ตั้งค่า PostgreSQL เพื่อให้ข้อมูล User คงอยู่ (แนะนำ)

ถ้าไม่ตั้ง `DATABASE_URL` ระบบจะใช้ SQLite ในดิสก์ของเซิร์ฟเวอร์ ซึ่ง **จะถูกลบเมื่อ instance หลับ/restart** ทำให้ User ที่สมัครไว้หายไป

**ขั้นตอน:**

1. ใน Render Dashboard คลิก **New +** → **PostgreSQL**
2. ตั้งชื่อ เช่น `ipo-readiness-db` → **Create Database**
3. รอสร้างเสร็จ แล้วไปที่ **Info** ของ database
4. คัดลอก **Internal Database URL** (หรือ External ถ้าใช้จากที่อื่น)
5. ไปที่ Web Service **ipo-readiness-mv** → **Environment**
6. เพิ่ม Environment Variable:
   - **Key**: `DATABASE_URL`
   - **Value**: วาง URL ที่คัดลอก (เช่น `postgresql://user:pass@host/dbname`)
7. **Save Changes** แล้วคลิก **Manual Deploy** → **Deploy latest commit**

หลังจากนั้น User และ Audit Log จะถูกเก็บใน PostgreSQL และ **จะไม่หาย** แม้เซิร์ฟเวอร์ restart
